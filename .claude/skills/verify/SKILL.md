...
- Duplicate injections (same rule+assets, same 5-min bucket) return action 'merged' (alert still new) or 'attached' (already promoted — evidence joins the existing investigation). A matching event after a close_fp completion returns 'reopened'.
- Integration tests need role passwords: `ALTER ROLE soctalk_admin PASSWORD ... SUPERUSER; ALTER ROLE soctalk_app PASSWORD ...; ALTER ROLE soctalk_mssp PASSWORD ...` then set DATABASE_URL_ADMIN/APP/MSSP.
- Bootstrap an admin with `soctalk-auth create-admin <email>` (DATABASE_URL_MSSP must be set).
