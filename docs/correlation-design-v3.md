...
## Additional notes on OS package verification

Please note that the OS packages (`.deb` and `.rpm`) do not include checksums in the release. This means that users cannot verify the integrity of these packages. However, the VM images do include checksums, which can be verified using the `SHA256SUMS.txt` file generated by the `build-packer-images.yml` workflow.